...
export default class OperationContainer extends PureComponent {
  ...
  // NTAP Modification - enforce authentication before try-it-out
  // to prevent browsers filling in passwords on try-it-out
  // except when this is bypassed via bypassYamlSecurity flag (e.g. in deploy swagger ui).
  onTryoutClick =() => {
    let {
      specActions,
      specSelectors,
    ...
    const { bypassYamlSecurity } = this.props.getConfigs()
    const security = op.getIn(["operation", "security"]) || specSelectors.security()
    if (bypassYamlSecurity) {
      this.setState({tryItOutEnabled: !this.state.tryItOutEnabled})
      specActions.clearValidateParams([path, method])
    }
    else if (!authSelectors.isAuthorized(security)) {
