...
export const sampleFromSchema = (schema, config={}) => {
  ...
  if(type === "object") {
    ...
    for (var name in props) {
      ...
      if ( props[name] &&
           ( props[name].readOnly || props[name]["x-ntap-readOnly"] ) &&
           !includeReadOnly
         ) {
        continue
      }
      if ( props[name] &&
           ( props[name].writeOnly || props[name]["x-ntap-writeOnly"] ) &&
           !includeWriteOnly
         ) {
        continue
      }
      if ( props[name] && props[name]["x-ntap-readCreate"] && !includeReadCreate ) {
        continue
      }
      if ( props[name] && props[name]["x-ntap-readModify"] && !includeReadModify ) {
        continue
      }
      if ( props[name] && props[name]["x-ntap-createOnly"] && !includeCreateOnly ) {
        continue
      }
      if ( props[name] && props[name]["x-ntap-modifyOnly"] && !includeModifyOnly ) {
        continue
      }
...
export const sampleXmlFromSchema = (schema, config={}) => {
  ...
  if (type === "object") {
    ...
    for (let propName in props) {
      ...
      if ( props[propName] &&
           ( props[propName].readOnly || props[propName]["x-ntap-readOnly"] ) &&
           !includeReadOnly
         ) {
        continue
      }
      if ( props[propName] &&
           ( props[propName].writeOnly || props[propName]["x-ntap-writeOnly"] ) &&
           !includeWriteOnly
         ) {
        continue
      }
      if ( props[propName] && props[propName]["x-ntap-readCreate"] && !includeReadCreate ) {
        continue
      }
      if ( props[propName] && props[propName]["x-ntap-readModify"] && !includeReadModify ) {
        continue
      }
      if ( props[propName] && props[propName]["x-ntap-createOnly"] && !includeCreateOnly ) {
        continue
      }
      if ( props[propName] && props[propName]["x-ntap-modifyOnly"] && !includeModifyOnly ) {
        continue
      }
