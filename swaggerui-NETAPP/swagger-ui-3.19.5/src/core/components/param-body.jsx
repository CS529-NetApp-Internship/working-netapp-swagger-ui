...
export default class ParamBody extends PureComponent {
  ...
  sample = (xml) => {
    let { pathMethod, param, fn:{inferSchema} } = this.props
    let schema = inferSchema(param.toJS())

    let ro = true, wo = true, co = true, mo = true, rc = true, rm = true

    if ( pathMethod.indexOf("get") > -1 ) {
        wo = false
        co = false
        mo = false
    } else if ( pathMethod.indexOf("patch") > -1 ) {
        ro = false
        co = false
        rc = false
    } else if ( pathMethod.indexOf("post") > -1 ) {
        ro = false
        mo = false
        rm = false
    }
    ...
    return getSampleSchema(schema, xml, {
      includeReadOnly: ro,
      includeWriteOnly: wo,
      includeCreateOnly: co,
      includeModifyOnly: mo,
      includeReadCreate: rc,
      includeReadModify: rm
    })
  }
